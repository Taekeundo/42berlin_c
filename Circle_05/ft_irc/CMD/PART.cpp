	...
	for (size_t i = 0; i < tmp.size(); i++){
		...
		for (size_t j = 0; j < this->channels.size(); j++){ // search for the channel
			if (this->channels[j].getName() == tmp[i]){ // check if the channel exist
				flag = true;
				if (!channels[j].getClient(fd) && !channels[j].getAdmin(fd)) // ERR_NOTONCHANNEL (442) // if the client is not in the channel
					{senderror(442, GetClient(fd)->GetNickName(), "#" + tmp[i], GetClient(fd)->GetFd(), " :You're not on that channel\r\n"); continue;}
				
				if ((userIsTheOnlyAdmininChannels(channels[i],fd)) && (channels[i].getAllClients().size() != 0))
					{_sendResponse(MSG_MSG(std::string(RED) + "Before you leave Channel <#" + channels[i].getName() + "> set a give adminrights to another user!" + std::string(RESET)),fd);}
				else{
					std::stringstream ss;
					...
					channels[j].broadcastMessage(ss.str());
					if (Flag_PrintInfoServer == 1)
						{std::cout << printBackground(LBLU,BWHI,"PART") << BLU << getClientNbrNick(fd) << " leave Channel #" << channels[i].getName() << std::endl;}
					if (channels[j].getAdmin(channels[j].findClientByName(GetClient(fd)->GetNickName())->GetFd()))
						channels[j].removeAdmin(channels[j].findClientByName(GetClient(fd)->GetNickName())->GetFd());
					else
						channels[j].removeClient(channels[j].findClientByName(GetClient(fd)->GetNickName())->GetFd());
