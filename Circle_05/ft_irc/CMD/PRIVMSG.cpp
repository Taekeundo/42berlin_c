...
void	Server::CheckForChannels_Clients(std::vector<std::string> &tmp, int fd)
{
	for(size_t i = 0; i < tmp.size(); i++){
		if (tmp[i][0] == '#'){
			...
			else if (!GetChannel(tmp[i])->findClientByName(GetClient(fd)->GetNickName())) //ERR_CANNOTSENDTOCHAN (404) // if the client is not in the channel
...
void	Server::PRIVMSG(std::string cmd, int fd)
{
	...
	for (size_t i = 0; i < tmp.size(); i++){// send the message to the clients and channels
		if (tmp[i][0] == '#'){
			tmp[i].erase(tmp[i].begin());
			std::string resp = std::string(MAG) + ":" + GetClient(fd)->GetNickName() + "!~" + GetClient(fd)->GetUserName() + "@localhost MSG to CHANNEL #" + tmp[i] + " ->" + message + std::string(RESET) + "\r\n";
			GetChannel(tmp[i])->broadcastMessage(resp, fd);
			if (Flag_PrintInfoServer == 1)
				std::cout << printBackground(LBLU,BWHI,"PRIVMSG") << MAG <<  getClientNbrNick(fd) << " send MSG to channel #" << tmp[i] << " ->" << message << std::string(RESET) << std::endl;
		}
		else{
			std::string resp = std::string(CYN) + ":" + GetClient(fd)->GetNickName() + "!~" + GetClient(fd)->GetUserName() + "@localhost PRIVMSG " + tmp[i] + " ->" + message + std::string(RESET) + "\r\n";
			_sendResponse(resp, GetClientNick(tmp[i])->GetFd());
			if (Flag_PrintInfoServer == 1)
				std::cout << printBackground(LBLU,BWHI,"PRIVMSG") << CYN << getClientNbrNick(fd) << " send PRIVMSG to nick: " << tmp[i] << " ->" << message << std::string(RESET) << std::endl;
		}
	}
}
